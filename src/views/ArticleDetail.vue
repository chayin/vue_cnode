<template>
  <div class="article-box" :style="{minHeight: minHeight + 50 + 'px'}">
    <div class="head-box">
      <h2 class="head-title" v-text="article.title"></h2>
    ...
    <div class="comment-box">
      ...
      <div class="comment-li" v-for="(c, index) in displayCommentList" :key="index">
        ...
        <div class="comment-content markdown-body" v-text="c.content">
        </div>
      </div>
    </div>
    <!-- 回复对话框 -->
    <popup v-model="popup.show" @on-hide="popup.show = false">
      <div class="popup-editor-box">
        <x-input class="weui_vcode weui_text_left" style="text-align: left;" v-model="popup.content" :placeholder="popup.placeholder" ref = "input">
    ...
    onLikeThisArticle () {
      if (this.checkLogin()) {
        ...
        this.$axios.post(url, {
          ...
          .then(result => {
            console.log(result)
            this.article.is_collect = !this.article.is_collect // 记住要取反
            this.$vux.toast.show({
              text: toastText
            })
          })
        ...
        this.$axios.post(`/reply/${id}/ups`, {
          ...
          .then(result => {
            ...
            if (result.data.action === 'down') {
              toastText = '取消点赞成功'
              let removeIndex = this.comment[index].ups.indexOf(this.$store.getters.loginInfo.id) // indexOf() 方法可返回某个指定的字符串值在字符串中首次出现的位置
